 ...
 * 基于马尔科夫假设的条件概率：A->B->C即下一个的概率只和前一个的概率有关系
       P(A,B,C) = P(C|B)*P(B|A)*P(A),因为C发生的概率和A没有关系。只和B相关
       P(B|C,A) = P(B|A)*P(C|B),因为C发生的概率和A没有关系，只和B相关。
 * 高斯分布的KL散度公式。
 * 参数重整化。
       若希望从高斯分布N(μ,σ)中采样，可以先从标准的正太分布N(0, 1)中采样z，
       再得到σ*z+μ，这样做的好处是将随机型转移到了z这个常量上，而σ和μ则当做是
       仿射变换网络的一部分。
